...
# Building a Python-based tool that:
#     Tracks:
#     File Creation
#         When a new file is created in a monitored directory.
#     File Access
#         When a file is opened/read by any user or process
#     File Deletion
#         When a file is removed.

# # Logs:
#     These events are logged with details like:

#         Timestamp

#         Type of action (created, accessed, deleted)

#         File name/path

#         Username or process (if possible)

# Security Aspect:
# Logs must be:

    # Stored securely (e.g., with proper permissions, or encryption if needed)

    # Protected from tampering

    # Managed using Python’s built-in logging module
